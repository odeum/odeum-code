...
const ExportModal = ({ exportModalIsOpen, closeExportModal, customStyles, appendix, frame, onClickExportFrame }) => {
	return (
		<div>
			{frame !== null && appendix !== null ?
				<ModalWindow isOpen={exportModalIsOpen} onRequestClose={closeExportModal} contentLabel="Eksportér til plansystem">
					...
					<ModalContent>
                        ...
                        Ramme {frame.name} indmeldes med følgende status: {appendix.status}
							<ModalButtonPanel>                            
								<Button id="exportButton" onClick={async (e) => { e.preventDefault(); await onClickExportFrame() }} icon={iconname.ICON_CLOUD_UPLOAD} size={18}>Eksportér</Button>
							</ModalButtonPanel>
						</div>
